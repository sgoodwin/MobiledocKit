...
class MobileDocTests: XCTestCase {
    ...
    func testCreatingMobileDocFromJSON()  {
        ...
        let card = try? decoder.decode([MobiledocMarkdownCard].self, from: data)
        XCTAssertNotNil(card)
        
        let notacard =  try? decoder.decode([MobiledocMarkdownCard].self, from: "[{\"title\":\"sup\"]".data(using: .utf8)!)
    ...
    func testMobileMarkdownCardDecoding() {
        ...
        do {
            ...
            let _ = try decoder.decode(MobiledocMarkdownCard.self, from: rawCard)
        } catch{
    ...
    func testInvalidMobileMarkdownCardDecoding() {
        ...
        let decoder = JSONDecoder()
        let rawCard = try! Data(contentsOf: url)
        XCTAssertThrowsError(try decoder.decode(MobiledocMarkdownCard.self, from: rawCard))
    }
    ...
    func testDecodingSections() {
        ...
        guard let section = mobiledoc.sections[0] as? MobiledocMarkerSection else {
